...
class Reranker:
    def __init__(self):
        ...
        self.base_url = "https://ai.api.nvidia.com/v1/retrieval/nvidia/reranking"
        self.model = "nvidia/nv-rerank-qa-mistral-4b:1"
        self.headers = {
    ...
    def rerank(
        ...
        try:
            ...
            rankings_sorted = sorted(rankings, key=lambda x: x["logit"], reverse=True)
            
            # Return top_k reranked results
            reranked_results = []
            for rank in rankings_sorted[:top_k]:
