...
from chat_debug_logger import ChatDebugLogger
...
async def generate_sql(request: QueryRequest):
    ...
    try:
        ...
        valid_tables, _ = prompt_builder.validate_tables(expanded_tables)
        ...
        sql_query = sql_generator.clean_sql(sql_query, request.query)
...
async def chat(request: QueryRequest):
    ...
    """
    debug_logger = ChatDebugLogger(request.query, request.context)
    ...
    if not all([qdrant_manager, graph_logic, prompt_builder, sql_generator, query_executor, result_formatter]):
        ...
        debug_logger.add_error("initialization", "Managers not initialized")
        log_path = debug_logger.write("error")
        print(f"📝 Chat debug log written: {log_path}")
    ...
    try:
        ...
        search_results = qdrant_manager.search(request.query, limit=15)
        debug_logger.add_qdrant_search(search_results, limit=15)

        # Step 2: RERANK to get top 5 most relevant tables
        if reranker and len(search_results) > 0:
            reranked_results = reranker.rerank(request.query, search_results, top_k=5)
            debug_logger.add_rerank(
                reranked_results=reranked_results,
                original_count=len(search_results),
                used_fallback=getattr(reranker, "last_rerank_used_fallback", False),
                error=getattr(reranker, "last_error", None),
                model=getattr(reranker, "model", None),
                base_url=getattr(reranker, "base_url", None),
            )
            if getattr(reranker, "last_rerank_used_fallback", False):
                print(f"⚠️  Reranker fallback active, using Qdrant order → top {len(reranked_results)}")
            else:
            ...
            debug_logger.add_rerank(
                ...
                model=getattr(reranker, "model", None) if reranker else None,
                base_url=getattr(reranker, "base_url", None) if reranker else None,
            )
        ...
        expansion = graph_logic.expand_search_results(reranked_results, depth=1)
        expanded_tables = expansion.get('all_tables', [])
        valid_tables, invalid_tables = prompt_builder.validate_tables(expanded_tables)
        tables_to_use = valid_tables or expansion.get('primary_tables', [])
        debug_logger.add_graph_expansion(expansion)

        # Step 4: Generate SQL
        prompt = prompt_builder.build_prompt(request.query, tables_to_use)
        debug_logger.add_prompt(
            prompt,
            tables_to_use,
            prompt_source="graph_expansion_all_tables",
            excluded_tables=invalid_tables,
        )

        raw_sql_query = sql_generator.generate_sql(prompt)
        sql_query = sql_generator.clean_sql(raw_sql_query, request.query)
        debug_logger.add_sql_generation(raw_sql_query, sql_query)

        # Validate SQL
        is_valid, error = sql_generator.validate_sql(sql_query)
        debug_logger.add_sql_validation(is_valid, error)
        if not is_valid:
            debug_logger.add_error("sql_validation", error or "Unknown validation error")
            log_path = debug_logger.write("sql_validation_failed")
            print(f"📝 Chat debug log written: {log_path}")
            return Phase3Response(
                query=request.query,
                tables_used=tables_to_use,
        ...
        debug_logger.add_execution(exec_result)

        if not exec_result['success']:
            debug_logger.add_error("query_execution", exec_result['error'] or "Unknown execution error")
            log_path = debug_logger.write("execution_failed")
            print(f"📝 Chat debug log written: {log_path}")
            return Phase3Response(
                query=request.query,
                tables_used=tables_to_use,
        ...
        debug_logger.add_answer(
            answer,
            used_fallback=getattr(result_formatter, "last_used_fallback", False),
            error=getattr(result_formatter, "last_error", None),
            model=getattr(result_formatter, "model", None),
        )
        log_path = debug_logger.write("success")
        print(f"📝 Chat debug log written: {log_path}")
        ...
        debug_logger.add_error("phase3_exception", str(e))
        log_path = debug_logger.write("error")
        print(f"📝 Chat debug log written: {log_path}")
